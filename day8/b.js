...
function example(input) {
  ...
  for (let i = 0; i < 140000; i++) {
    ...
    if (circuts.length === 1 && circuts[0].length === input.length) {
      console.log("lastConnection", lastConnection);

      const p1 = lastConnection[0].split(",").map(Number);
      const p2 = lastConnection[1].split(",").map(Number);
  ...
  return sum; // 25272 // 216,146,977 and 117,168,530
}
